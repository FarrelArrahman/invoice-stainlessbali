                    ...
                    <div class="col-12">
                        <form action="{{ route('technicians.update', $technician->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text"
                                    class="form-control" id="name" name="name" value="{{ $technician->name }}">
                            </div>
                            <div class="mb-3">
                                <label for="address" aria-describedby="addressHelp">Alamat</label>
                                <input type="text"
                                    class="form-control" id="address" name="address" value="{{ $technician->address }}">
                            </div>
                            <div class="mb-3">
                                <label for="phone_number">Nomor Telepon</label>
                                <input type="text"
                                    class="form-control" id="phone_number" name="phone_number" value="{{ $technician->phone_number }}">
                            </div>
                            <div class="mb-3">
                                <label for="status">Status</label>
                                <select name="status" id="status" class="form-select">
                                    @foreach(\App\Enums\StatusEnum::activeCases() as $status)
                                    <option @selected($status->value == $technician->status) value="{{ $status->value }}">{{ $status->name }}</option>
                                    @endforeach
                                </select>
                            </div>
